...
class FlutterBluePlus {
  ...
  static Stream<L2CapChannelConnected> get l2CapChannelConnected {
    _l2CapChannelConnected ??= FlutterBluePlusPlatform.instance.onL2CapChannelConnected
        .map((p) => L2CapChannelConnected.fromProto(p));
  ...
  static Future<int> listenL2CapChannel({
    ...
    return await _invokeMethod(() => 
        FlutterBluePlusPlatform.instance.listenL2CapChannel(request))
        .then((r) => r.psm);
  }
  ...
  static Future<void> closeL2CapServer({required final int psm}) async {
    ...
    return await _invokeMethod(() => 
        FlutterBluePlusPlatform.instance.closeL2CapServer(request));
  }
...
class L2CapChannelConnected {
  ...
  L2CapChannelConnected({
    required this.device,
    required this.psm,
  });

  factory L2CapChannelConnected.fromProto(BmL2CapChannelConnected p) {
    return L2CapChannelConnected(
      device: BluetoothDevice.fromId(p.device.remoteId.str),
      psm: p.psm,
    );
  }
}
